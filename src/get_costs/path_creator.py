...
def get_JOURNAL_DATA_PATH(key, year, journal_name):
    first_path_part = config.get_paths_for(key).get('JOURNAL_DATA_PATH')
    if key:
...
def get_EMPLOYEE_DATA_PATH(key, year):
    first_path_part = config.get_paths_for(key).get('RAW_DATA_PATH')
...
def get_RESULT_FILE_PATH(key, year_month_prefix):
    first_path_part = config.get_paths_for(key).get('RESULT_FILE_PATH')
    if key:
