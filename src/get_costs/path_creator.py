    ...
    for path in file_list:
        if not os.path.exists(path):
            raise PathNotFoundError(
                f'Die Datei "{path}" existiert unter dem angegebenen Pfad nicht. Bitte lege sie an oder ändere den Pfad in der config.py. Dann starte das Programm neu.')
...
def get_paths(key, year, journal_name, year_month_prefix):
    ...
    validate_path_exists([journal_data_file, employee_data_file,
