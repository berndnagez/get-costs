...
def get_social_insurance(project):
    if not project.get('KV-AG-Beitrag') and not project.get('RV-AG-Beitrag') and not project.get('AV-AG-Beitrag') and not project.get('PV-AG-Beitrag'):
        return 0.0
    insurance = float(project.get('KV-AG-Beitrag')) + float(project.get('RV-AG-Beitrag')) + \
        float(project.get('AV-AG-Beitrag')) + \
        float(project.get('PV-AG-Beitrag'))
...
def get_surcharges(project):
    if not project.get('U1 - Umlage 1') and not project.get('U2 - Umlage 2') and not project.get('InsoU - Insolvenzgeldumlage'):
        return 0.0
    umlagen = float(project.get('U1 - Umlage 1')) + float(project.get('U2 - Umlage 2')
                                                          ) + float(project.get('InsoU - Insolvenzgeldumlage'))
    return str(umlagen)
...
def split_costs(projectlist, projects):
    ...
    for project in projectlist:
        ...
        if projects:
            ...
            keys_and_value_list = get_keys_and_value_list_of_projects(
                first_name, project)
            create_formulas_for_projects(
            ...
            splitted_values_dict['project_hours'] = project.get(
                'project_hours')
            keys_and_value_list = get_keys_and_value_list_of_project(project)
            if project.get('ID') == ID_OF_ACCOUNTING_PERSON:
                create_formulas_for_accounting_person(
                    project_id, keys_and_value_list, get_divisor_accounting_person(project), splitted_values_dict)
            elif project.get('ID') == ID_OF_CLEANING_PERSON:
                create_formulas_for_cleaning_person(
                    project_id, keys_and_value_list, splitted_values_dict)
            else:
                create_formulas_for_project(
                    keys_and_value_list, divisor, splitted_values_dict)
