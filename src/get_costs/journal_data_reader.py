...
def get_data_frame(filename):
    ...
    try:
        ...
        print(f'FEHLER: "Pers.Nr." konnte in "{filename}" nicht gefunden werden.')
...
def get_sheet(filename):
    ...
    if len(sheet_names) > 1:
        if not sheet_name in sheet_names:
            raise SheetNotFoundError(f"Sheet '{sheet_name}' not found in document '{filename}'!")
    else:
...
def get_cell_value(file, df, personal_num, column_name):
    try:
        ...
        if cell_value == 0.0 and column_name == 'St.Brutto - Steuerbrutto':
            cell_value = df.loc[personal_num, 'BezPausch - Pauschal versteuerte Bezüge']
        return cell_value
    except KeyError:
        print(f'FEHLER: "{personal_num}" oder "{column_name}" konnte in "{file}" nicht gefunden werden.')
...
def get_sum_of_social_insurance(file, df, personal_num):
    ...
    for column in config.COMPONENTS_OF_SOCIAL_INSURANCE:
        sum_of_social_insurance += get_cell_value(file, df, personal_num, column)
...
def get_all_values_of_employee(file, df, personal_num, columns):
    ...
    for key, value in columns.items():
        ...
        if key == "Entgelt":
            all_values['Sozialvers.']=(get_sum_of_social_insurance(file, df, personal_num))
    return all_values
...
def get_values_of_all_employees(file, df, nums, column_names_of_interest):
    all_employees = []
    for num in nums:
        all_employees.append(get_all_values_of_employee(file, df, num, column_names_of_interest))
...
def show_debug_infos():
    print(get_values_from("23_05_24 Lohnjournal Mai 2023.xlsx", [1004, 1007, 1028]))
